...
for state in states:
    ...
    if len(years)>0:
        ...
        for product in years[0]["products"]:
           ws.cell(column=col, row=rowIndex, value=product["product"])
           col+=1
    rowIndex+=1
    for year in years:
        ...
        for product in year["products"]:
            ...
            col+=1
        rowIndex+=1
    data = Reference(ws, min_col=2, min_row=startRow, max_col=len(years[0]["products"])+1, max_row=rowIndex-1)
    c2 = LineChart()
    c2.title = "Sectorial Products at "+state["state"]+" State"
    c2.style = 12
